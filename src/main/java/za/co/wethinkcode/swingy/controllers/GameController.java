...
public class GameController
{
    ...
    public GameController(String view)
    {
        ...
        {
            System.out.println("Select a correct view.");
            System.exit(0);
        }
    ...
    private void createVillains()
    {
        ...
        for (int i = 0; i < numVillains; i++)
        {
            ...
            String names[] = {"Ninja","Condor", "Autre"}, phrases[] = {"I am the Brave", "this is for the brave", "Cool"};
    ...
    private String roundResults(Player one, Player two, boolean bothAttack)
    {
        ...
        if (bothAttack)
        {
            result = String.format("\n%s %s fight %s %s with %d damage points.\n", one.getType(), one.getName(),
                                    two.getType(), two.getName(), one.getAtk());
            result += String.format("%s %s fight %s %s with %d damage points.\n", two.getType(), two.getName(),
                                    one.getType(), one.getName(), two.getAtk());
            result += String.format("%s %s has got some %d damage points.\n", one.getType(), one.getName(),
                                    two.getAtk());
            result += String.format("%s %s has got some %d damage points.\n", two.getType(), two.getName(),
                                    one.getAtk());
        ...
        else
        {
            ...
            result = String.format("\n%s %s fight %s %s with %d damage points.\n", one.getType(), one.getName(),
                    two.getType(), two.getName(), one.getAtk());
            result += String.format("%s %s has defended %s %s with %d defense points.\n", two.getType(), two.getName(),
    ...
    public void receiveUserInput(String input)
    {
        switch (currentStage)
        {
            ...
            case SELECTION:
                ...
                {
                    retrieveHero(Integer.parseInt(input) - 1);
                    if (errors.size() != 0)
                        currentStage = gameStage.ERRORS;
                    else
                    {
    ...
    private String pickUpArtefact()
    {
        ...
        if (chances >= 7)
        {
            ...
            if (index == 0)
            {
                pickedArtefact = "\nYou have chosen a weapon which stands for "+ value +" dttack points\n";
                hero.setAtk(hero.getAtk() + value);
            }
            else if (index == 1)
            {
                pickedArtefact = "\nYou have chosen an Armor which adds "+ value +" defense points\n";
                hero.setDef(hero.getDef() + value);
            }
            else
            {
                pickedArtefact = "\nYou have chosen a Helm which adds "+ value +" hit points\n";
